...
import { Home, Wallet, Goal, User, Plus, CreditCard, CalendarRange } from 'lucide-react';
...
export const AppShell = ({ children }: { children?: React.ReactNode }) => {
    return (
        <div className="flex flex-col h-screen w-full bg-background text-foreground overflow-hidden">
            ...
            <nav className="md:hidden fixed bottom-6 left-4 right-4 h-16 bg-card/90 backdrop-blur-xl border border-white/10 rounded-2xl z-40 flex items-center justify-between px-6 shadow-2xl shadow-black/50">
                ...
                <NavItem to="/goals" icon={<Goal size={22} />} label="Metas" />
            ...
            <div className="md:hidden fixed bottom-10 left-1/2 -translate-x-1/2 z-50">
                <button
                    ...
                    className="flex h-14 w-14 items-center justify-center rounded-full bg-accent text-accent-foreground shadow-lg shadow-accent/40 hover:scale-105 active:scale-95 transition-all outline-none ring-4 ring-background"
                >
